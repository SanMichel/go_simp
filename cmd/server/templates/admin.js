...
async function loadUsers() {
  ...
  if (!ok || !Array.isArray(data)) {
    ...
    errDiv.className = "empty";
    errDiv.style.color = "var(--danger)";
    errDiv.textContent = "Erro ao carregar usuários.";
    errTd.appendChild(errDiv);
    errTr.appendChild(errTd);
    tbody.appendChild(errTr);
    return;
  ...
  if (user && user.role === "sysadmin") {
    ...
    const statGerentes = document.getElementById("stat-gerentes");
    if (statGerentes)
      statGerentes.textContent = data.filter((u) => u.role === "gerente").length.toString();
    const statConferentes = document.getElementById("stat-conferentes");
    if (statConferentes)
      statConferentes.textContent = data.filter((u) => u.role === "conferente").length.toString();
  }
  if (data.length === 0) {
    tbody.innerHTML = "";
    const emptyTr = document.createElement("tr");
    ...
    const emptyDiv = document.createElement("div");
    emptyDiv.className = "empty";
    emptyDiv.textContent = "Nenhum usuário encontrado.";
    emptyTd.appendChild(emptyDiv);
    emptyTr.appendChild(emptyTd);
    tbody.appendChild(emptyTr);
    return;
  ...
  tbody.innerHTML = "";
  for (const u of data) {
    const [dataRow, editRow] = buildUserRows(u);
    tbody.appendChild(dataRow);
    tbody.appendChild(editRow);
  }
}
function buildUserRows(u) {
  const canEdit = user && (user.role === "sysadmin" || user.role === "gerente" && u.role === "conferente");
  ...
  const badge = document.createElement("span");
  badge.className = `badge badge-${escHtml(u.role)}`;
  badge.textContent = rolePt(u.role);
  ...
  if (canEdit) {
    ...
    editBtn.setAttribute("data-user-id", String(u.id));
    editBtn.setAttribute("data-action", "toggle-edit");
  ...
  editRow.style.background = "var(--surface2)";
  const editCell = document.createElement("td");
  editCell.colSpan = 4;
  editCell.style.padding = "12px 14px";
  const inlineEdit = document.createElement("div");
  inlineEdit.className = "inline-edit open";
  inlineEdit.id = `inline-${u.id}`;
  const roleField = document.createElement("div");
  roleField.className = "form-field";
  const roleLabel = document.createElement("label");
  roleLabel.textContent = "Novo cargo";
  roleField.appendChild(roleLabel);
  const roleSelect = document.createElement("select");
  roleSelect.id = `edit-role-${u.id}`;
  const allowed = user && (user.role === "sysadmin" ? ["conferente", "gerente", "sysadmin"] : ["conferente"]);
  for (const r of allowed) {
    const opt = document.createElement("option");
    opt.value = r;
    opt.textContent = rolePt(r);
    if (r === u.role)
      opt.selected = true;
    roleSelect.appendChild(opt);
  }
  roleField.appendChild(roleSelect);
  inlineEdit.appendChild(roleField);
  const passField = document.createElement("div");
  passField.className = "form-field";
  const passLabel = document.createElement("label");
  passLabel.innerHTML = 'Nova senha <span style="color:var(--muted)">(opcional)</span>';
  passField.appendChild(passLabel);
  const passInput = document.createElement("input");
  passInput.type = "password";
  passInput.id = `edit-pass-${u.id}`;
  passInput.placeholder = "••••••••";
  passField.appendChild(passInput);
  inlineEdit.appendChild(passField);
  const saveBtn = document.createElement("button");
  saveBtn.className = "btn btn-primary";
  saveBtn.setAttribute("data-user-id", String(u.id));
  saveBtn.setAttribute("data-action", "save");
  saveBtn.textContent = "Salvar";
  inlineEdit.appendChild(saveBtn);
  const cancelBtn = document.createElement("button");
  cancelBtn.className = "btn btn-ghost";
  cancelBtn.setAttribute("data-user-id", String(u.id));
  cancelBtn.setAttribute("data-action", "cancel-edit");
  cancelBtn.textContent = "Cancelar";
  inlineEdit.appendChild(cancelBtn);
  editCell.appendChild(inlineEdit);
  const msgDiv = document.createElement("div");
  msgDiv.className = "msg";
  msgDiv.id = `edit-msg-${u.id}`;
  editCell.appendChild(msgDiv);
...
async function saveUser(id) {
  ...
  const passEl = document.getElementById(`edit-pass-${id}`);
  const msgEl = document.getElementById(`edit-msg-${id}`);
  const body = {};
  if (roleEl.value)
    body.role = roleEl.value;
  if (passEl.value.trim())
    body.password = passEl.value.trim();
  if (Object.keys(body).length === 0) {
    if (msgEl)
      setMsg(msgEl, "Nenhuma alteração informada.", false);
    return;
  }
  showLoader(true);
  ...
  if (ok) {
    if (msgEl)
      setMsg(msgEl, "✔ Salvo com sucesso!", true);
    setTimeout(() => {
      toggleEdit(id);
      loadUsers();
    }, 900);
    ...
    if (msgEl)
      setMsg(msgEl, data.error || "Erro ao salvar.", false);
  }
}

// src/client/admin/index.ts
setLogoutCb(logout);
setLoadUsersCb(loadUsers);
...
document.getElementById("form-admin-login")?.addEventListener("submit", async (e) => {
  ...
  showLoader(false);
  if (res.ok) {
    if (res.data.user.role !== "sysadmin") {
      if (errEl) {
        errEl.textContent = "Acesso restrito a sysadmin";
...
document.getElementById("form-create-user")?.addEventListener("submit", async (e) => {
  ...
  if (ok) {
    if (msgEl)
      setMsg(msgEl, `✔ Usuário "${data.username}" criado com sucesso!`, true);
    e.target.reset();
    loadUsers();
  } else if (msgEl)
    setMsg(msgEl, data.error || "Erro ao criar usuário.", false);
});
...
document.addEventListener("click", (e) => {
  const btn = e.target.closest('button[data-action="toggle-edit"], button[data-action="cancel-edit"], button[data-action="save"]');
  if (!btn)
    return;
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (sessionRes.ok && sessionRes.data?.user?.role === "sysadmin") {
    setToken("cookie");
